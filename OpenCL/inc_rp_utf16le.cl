...
{
  const int sd  = off_src / 2;
  const int sm  = off_src & 1;
  const int sm16 = sm * 16;

  const int dd  = off_dst / 2;
  const int dm  = off_dst & 1;
  const int dm16 = dm * 16;
...
DECLSPEC void append_three_byte_utf16le (PRIVATE_AS const u32 *buf_src, const int off_src, PRIVATE_AS u32 *buf_dst, const int off_dst) // OK a tester Doute sur le masque 
{
  const int sd  = off_src / 2;
  const int sm  = off_src & 1;
  const int sm16 = sm * 16;
...
DECLSPEC void append_two_byte_utf16le (PRIVATE_AS const u32 *buf_src, const int off_src, PRIVATE_AS u32 *buf_dst, const int off_dst) // OK a tester Doute sur le masque
{
  const int sd  = off_src / 2;
  const int sm  = off_src & 1;
  const int sm16 = sm * 16;
...
DECLSPEC void append_one_byte_utf16le (PRIVATE_AS const u32 *buf_src, const int off_src, PRIVATE_AS u32 *buf_dst, const int off_dst) // OK a tester  Doute sur le masque
{
  const int sd  = off_src / 2;
  const int sm  = off_src & 1;
  const int sm16 = sm * 16;
...
DECLSPEC void append_block_utf16le (PRIVATE_AS const u32 *buf_src, const int off_src, PRIVATE_AS u32 *buf_dst, const int off_dst, const int len) // OK a tester 
{
  int i;

  for (i = 0; i < len - 3; i += 2)
  {
    append_four_byte_utf16le (buf_src, off_src + i, buf_dst, off_dst + i);
  }
  ...
  switch (left)
  {
    case 3: append_three_byte_utf16le (buf_src, off_src + i, buf_dst, off_dst + i); break;
    case 2: append_two_byte_utf16le   (buf_src, off_src + i, buf_dst, off_dst + i); break;
    case 1: append_one_byte_utf16le   (buf_src, off_src + i, buf_dst, off_dst + i); break;
  }
}

DECLSPEC void exchange_byte_utf16le (PRIVATE_AS u32 *buf, const int off_src, const int off_dst) // OK a tester 
{
...
DECLSPEC int mangle_lrest_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    const u32 t = buf[idx];

    buf[idx] = t | generate_cmask_utf16le (t);
  }

  return (len);
}

DECLSPEC int mangle_lrest_ufirst_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester 
{
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    const u32 t = buf[idx];
  ...
  buf[0] = t & ~(0x00000020 & generate_cmask_utf16le (t));

  return (len);
}

DECLSPEC int mangle_urest_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    const u32 t = buf[idx];
...
DECLSPEC int mangle_urest_lfirst_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    const u32 t = buf[idx];
...
DECLSPEC int mangle_trest_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    const u32 t = buf[idx];
...
DECLSPEC int mangle_toggle_at_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester 
{
  ...
  const u32 tmp = 0x20u << (p0m * 16);

  const u32 t = buf[p0d];

  buf[p0d] = t ^ (generate_cmask_utf16le (t) & tmp);

  return (len);
}

DECLSPEC int mangle_toggle_at_sep_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // en cours 
{
  ...
  for (int i = 0, idx = 0; i < len; i += 2, idx += 1)
  {
    ...
    if (((t >>  0) & 0xffff) == p1) { if (occurence == p0) out = 0x0000ff00; occurence++; }
    if (((t >>  16) & 0xffff) == p1) { if (occurence == p0) out = 0x00ff0000; occurence++; }
    //if (((t >> 16) & 0xff) == p1) { if (occurence == p0) out = 0xff000000; occurence++; }
    //if (((t >> 24) & 0xff) == p1) { if (occurence == p0) rem = 0x000000ff; occurence++; }

    buf[idx] = t ^ (generate_cmask_utf16le (t) & out);
  }

  return (len);
}

DECLSPEC int mangle_reverse_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
...
DECLSPEC int mangle_dupeword_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester
{
  const int out_len = len * 2;

  if (out_len >= RP_PASSWORD_SIZE) return (len);

  append_block_utf16le (buf, 0, buf, len, len);

  return (out_len);
}

DECLSPEC int mangle_dupeword_times_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u16 *buf, const int len) // OK a tester, vérifier len 
{
...
DECLSPEC int mangle_reflect_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int len) // OK a tester 
{
  const int out_len = len * 2;
  ...
  for (int l = 0; l < len / 2; l++)
  {
    ...
    exchange_byte_utf16le (buf, len + l, len + r);
  }

  return out_len;
}

DECLSPEC int mangle_append_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u16 *buf, const int len) // OK a tester, vérifier len
{
...
DECLSPEC int mangle_prepend_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u16 *buf, const int len)  // OK a tester, vérifier len
{
  const int out_len = len + 1;
...
DECLSPEC int mangle_delete_first_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u16 *buf, const int len)  // OK a tester 
{
  return mangle_delete_at_utf16le (0, p1, buf, len);
}

DECLSPEC int mangle_delete_last_utf16le (MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u16 *buf, const int len) // OK a tester 
{
  if (len == 0) return 0;

  return mangle_delete_at_utf16le (len - 1, p1, buf, len);
}
...
DECLSPEC int apply_rule_utf16le (const u32 name, MAYBE_UNUSED const u16 p0, MAYBE_UNUSED const u16 p1, PRIVATE_AS u32 *buf, const int in_len)
{
  ...
  switch (name)
  {
    case RULE_OP_MANGLE_LREST:            out_len = mangle_lrest_utf16le            (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_LREST_UFIRST:     out_len = mangle_lrest_ufirst_utf16le     (p0, p1,        buf, out_len); break;
    ...
    case RULE_OP_MANGLE_DUPEWORD:         out_len = mangle_dupeword_utf16le         (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_DUPEWORD_TIMES:   out_len = mangle_dupeword_times_utf16le   (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    case RULE_OP_MANGLE_REFLECT:          out_len = mangle_reflect_utf16le          (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_APPEND:           out_len = mangle_append_utf16le           (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    case RULE_OP_MANGLE_PREPEND:          out_len = mangle_prepend_utf16le          (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    case RULE_OP_MANGLE_ROTATE_LEFT:      out_len = mangle_rotate_left_utf16le      (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_ROTATE_RIGHT:     out_len = mangle_rotate_right_utf16le     (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_DELETE_FIRST:     out_len = mangle_delete_first_utf16le     (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    case RULE_OP_MANGLE_DELETE_LAST:      out_len = mangle_delete_last_utf16le      (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    case RULE_OP_MANGLE_DELETE_AT:        out_len = mangle_delete_at_utf16le        (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
    ...
    case RULE_OP_MANGLE_SWITCH_FIRST:     out_len = mangle_switch_first_utf16le     (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_SWITCH_LAST:      out_len = mangle_switch_last_utf16le      (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_SWITCH_AT:        out_len = mangle_switch_at_utf16le        (p0, p1,        buf, out_len); break;
    case RULE_OP_MANGLE_CHR_SHIFTL:       out_len = mangle_chr_shiftl_utf16le       (p0, p1, (PRIVATE_AS u16 *) buf, out_len); break;
...
DECLSPEC int apply_rules_utf16le (CONSTANT_AS const u64 *cmds, PRIVATE_AS u32 *buf, const int in_len)
{
  ...
  for (u32 i = 0; cmds[i] != 0; i++)
  {
    ...
    const u16 name = (cmd >>  0) & 0xffff;
    const u16 p0   = (cmd >>  16) & 0xffff;
    const u16 p1   = (cmd >> 32) & 0xffff;
    ...
    out_len = apply_rule_utf16le (name, p0, p1, buf, out_len);
  }
