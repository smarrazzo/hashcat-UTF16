#hashcat.exe -a 0 -m 143 ..\test\test_rules\h.txt ..\test\test_rules\dico.txt -r ..\test\test_rules\rules.txt --potfile-disable -O --encoding-to=utf16le -u1 -n1 -Y 1 --backend-vector=1  --force
...
for word in words:
    try:
        h = SHA1.new()
        h.update(salt.encode('utf-8')+word.encode('utf-16le'))
        tmp = b"$episerver$*0*"+b64.b64encode(salt.encode('utf8'))+b"*"+b64.b64encode(h.digest())
        f.write(tmp+b'\n')
        ff.write(tmp.decode('utf8')+':'+word+'\n')
