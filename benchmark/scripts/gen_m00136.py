#hashcat.exe -a 0 -m 136 ..\test\test_rules\h.txt ..\test\test_rules\dico.txt -r ..\test\test_rules\rules.txt --potfile-disable -O --encoding-to=utf16le -u1 -n1 -Y 1 --backend-vector=1  --force
...
for word in words:
    try:
        h = SHA1.new()
        h.update(word.encode('utf-16le')+salt.encode('utf-8'))
        f.write("0x0100"+"".join("{:02x}".format(ord(c)) for c in salt)+h.hexdigest()+'\n')
        ff.write("0x0100"+"".join("{:02x}".format(ord(c)) for c in salt)+h.hexdigest()+':'+word+'\n')
